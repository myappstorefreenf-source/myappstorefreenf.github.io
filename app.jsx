...
const GlobalConfig = {
    ...
    whatsNewVersion: "2026-08-11",
    whatsNewMessage: `• DISPONIBLE PLAY TV PREMIUM V20.1.
• NUEVA VERSION DE PUNTO PLAY.`
};
...
const apps = [
    ...
    {            
     ...
     {            
        ...
        icon: 'icons/carg.png',            
        description: 'Disfruta de la programacion en vivo de Argentina con esta excelente app  ' ,            
    ...
    {            
     ...
     {            
        ...
        publisher: 'MagLink ',            
            icon: 'icons/mag.png',            
        description: 'Magma es un reproductor de listas m3u, requiere una minima configuracion inicial, al pedirte los datos ingresa servidor:http://magmaplayer.com/ usuario: m contraseña: m y disfruta de todo el contenido .',            
        downloadUrl: 'https://github.com/myappstorefreenf-source/mis-apps-android/releases/download/1.0.0.0/MagLink.TV.1.5.0.apk',            
        isAvailable: true,        
    },        
     {            
        id: 'Punto play ',            
...
// NUEVO: Componente Modal Novedades Web
const WhatsNewModal = ({ isVisible, onClose, message }) => {
    if (!isVisible) return null;

    return (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-90 backdrop-blur-sm flex items-center justify-center p-6 z-50">
            <div className="bg-gray-800 rounded-3xl p-8 max-w-lg w-full shadow-2xl relative border-2 border-purple-500 text-center">
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 text-gray-400 hover:text-white focus:text-white focus:bg-gray-700 rounded-full w-10 h-10 flex items-center justify-center text-2xl font-bold transition-colors outline-none focus:ring-2 focus:ring-purple-500"
                >
                    &times;
                </button>
                <div className="text-5xl mb-4 animate-bounce">🚀</div>
                <h3 className="text-3xl font-extrabold text-white mb-4">¡Novedades del Sistema!</h3>
                <p className="text-purple-300 text-base mb-4 font-semibold">Enterate de los últimos cambios implementados:</p>
                <div className="text-gray-200 text-sm leading-relaxed mb-8 bg-gray-900 bg-opacity-60 p-5 rounded-2xl text-left border border-gray-700 whitespace-pre-line">
                    {message || "Se aplicaron mejoras generales de estabilidad en la plataforma."}
                </div>
                <button
                    onClick={onClose}
                   // autoFocus
...
>
    {selectedApp.isAvailable ? 'DESCARGAR AHORA' : 'No Disponible'}
            ...
            {showPasswordModal && (
                ...
                {/* NUEVO: Integración del Modal de Novedades del sistema web */}
            <WhatsNewModal 
